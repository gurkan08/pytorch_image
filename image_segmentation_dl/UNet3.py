...
class UNet2(torch.nn.Module):
    def __init__(self):
        ...
        #################### MIDDLE BLOCK #########################################
        self.middle_block_conv1 = torch.nn.Conv2d(128, 256, 3, padding=1)
        self.middle_block_bn1 = torch.nn.BatchNorm2d(256)
        self.middle_block_conv2 = torch.nn.Conv2d(256, 256, 3, padding=1)
        self.middle_block_bn2 = torch.nn.BatchNorm2d(256)
        self.middle_block_conv3 = torch.nn.Conv2d(256, 256, 3, padding=1)
        self.middle_block_bn3 = torch.nn.BatchNorm2d(256)
        ...
        self.up_block1_conv1 = torch.nn.Conv2d(128 + 256, 128, 3, padding=1)
        self.up_block1_bn1 = torch.nn.BatchNorm2d(128)
        self.up_block1_conv2 = torch.nn.Conv2d(128, 128, 3, padding=1)
        self.up_block1_bn2 = torch.nn.BatchNorm2d(128)
        self.up_block1_conv3 = torch.nn.Conv2d(128, 128, 3, padding=1)
        self.up_block1_bn3 = torch.nn.BatchNorm2d(128)
        self.up_block1_relu = torch.nn.ReLU()

        # up_block2
        self.up_block2_up_sampling = torch.nn.Upsample(scale_factor=2, mode='bilinear')
        self.up_block2_conv1 = torch.nn.Conv2d(64 + 128, 64, 3, padding=1)
        self.up_block2_bn1 = torch.nn.BatchNorm2d(64)
        self.up_block2_conv2 = torch.nn.Conv2d(64, 64, 3, padding=1)
        self.up_block2_bn2 = torch.nn.BatchNorm2d(64)
        self.up_block2_conv3 = torch.nn.Conv2d(64, 64, 3, padding=1)
        self.up_block2_bn3 = torch.nn.BatchNorm2d(64)
        self.up_block2_relu = torch.nn.ReLU()

        # up_block3
        self.up_block3_up_sampling = torch.nn.Upsample(scale_factor=2, mode='bilinear')
        self.up_block3_conv1 = torch.nn.Conv2d(32 + 64, 32, 3, padding=1)
        self.up_block3_bn1 = torch.nn.BatchNorm2d(32)
        self.up_block3_conv2 = torch.nn.Conv2d(32, 32, 3, padding=1)
        self.up_block3_bn2 = torch.nn.BatchNorm2d(32)
        self.up_block3_conv3 = torch.nn.Conv2d(32, 32, 3, padding=1)
        self.up_block3_bn3 = torch.nn.BatchNorm2d(32)
        self.up_block3_relu = torch.nn.ReLU()

        # up_block4
        self.up_block4_up_sampling = torch.nn.Upsample(scale_factor=2, mode='bilinear')
        self.up_block4_conv1 = torch.nn.Conv2d(16 + 32, 16, 3, padding=1)
        self.up_block4_bn1 = torch.nn.BatchNorm2d(16)
        self.up_block4_conv2 = torch.nn.Conv2d(16, 16, 3, padding=1)
        self.up_block4_bn2 = torch.nn.BatchNorm2d(16)
        self.up_block4_conv3 = torch.nn.Conv2d(16, 16, 3, padding=1)
        self.up_block4_bn3 = torch.nn.BatchNorm2d(16)
    ...
    def forward(self, x):
        ...
        # middle_block
        x5 = self.middle_block_relu(self.middle_block_bn1(self.middle_block_conv1(x4)))
        x5 = self.middle_block_relu(self.middle_block_bn2(self.middle_block_conv2(x5)))
        x5 = self.middle_block_relu(self.middle_block_bn3(self.middle_block_conv3(x5)))
        # print("middle:", x7.shape)

        # up_block1
        x = self.up_block1_up_sampling(x5)
        # print("up1:", x.shape)
        x = torch.cat((x, self.x4), dim=1)  # .cat(x,prev_feature_map=x6)
        # print("cat:",x.shape)
